...
public class DataRepository : DataHandler {
	...
	*/
	...
	private static Godot.Collections.Dictionary<string,Variant> SetupDefaultData() {
		...
		string JsonText = File.ReadAllText(Path.Join(ProjectSettings.GlobalizePath("res://"),"Scripts/Backend/DefaultData.json"));
	...
	*/
	...
	public static Godot.Collections.Dictionary<int,Variant> CompilePlacedItems() {
		...
		foreach(Node3D PlacedItem in PlacingGridHandler.NODE_PLACED_STORAGE.GetChildren()) {
	...
	public static void DecompilePlacingData() {
		...
		foreach((int _, Variant Data) in PlacementData) {
			...
			// Blocks somehwere here
			// Prevents other shit from loading :|
			// Crop name doesnt exist
			// Nvm was loading empty crops pffffttt
			
			// does stuff for unique meta data
			foreach(string MetaDataEntry in MetaDataList) {
				// Crop Garbage
				if(MetaDataEntry == "PlantType") {
					// Gather Crop Data
					string CropName = DataCorrected[ENCRYPTION_LIBRARY["PlantType"]].ToString();
					int TimeRemaining = (int)Int32.Parse(DataCorrected[ENCRYPTION_LIBRARY["TimeRemaining"]].ToString());
					int CropStage = (int)Int32.Parse(DataCorrected[ENCRYPTION_LIBRARY["PlantStage"]].ToString());

					// Fuck empty crops, bane of my existence
					// did it again, loading empty crops :]
					if(CropName != "") {
						// Create a new crop	
						ulong? ObjectID = CropController.BindCropToObject("Wheat",PlacedItem);
						CropController.OverrideCropData(CropStage,TimeRemaining,ObjectID);
