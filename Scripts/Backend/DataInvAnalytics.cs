...
public partial class DataInvAnalytics : DataRepository {
    // Public shit
    ...
    public static void BufferInventoryData() {
        ...
        foreach((string TypeName, Variant _) in OverrideHistory) {
            ...
            if(DecryptedData == "") {
                GD.PushWarning("Solving lost memory of " + TypeName);
                // Ik its not the most safe thing
                // if shit hits the fan, look here
                if(((Godot.Collections.Dictionary<string,int>)((Godot.Collections.Dictionary<string,Variant>)DataHandler.DATA_FILE["I"])["A"]).ContainsKey(TypeName)) {
                    ((Godot.Collections.Dictionary<string,int>)((Godot.Collections.Dictionary<string,Variant>)DataHandler.DATA_FILE["I"])["A"]).Remove(TypeName);
                }
            }
        }
    }
