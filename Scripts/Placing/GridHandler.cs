...
public partial class GridHandler : Node
{
	// Paths
    ...
    public override void _Process(double _DeltaTime) {
		...
		if(Input.IsActionJustPressed("Placing_Place") && IS_PLACING && GetViewport().GuiGetFocusOwner() == null) {
			PLACING_SYSTEM.Place(GridSystem3D.CURRENT_ITEM_HITBOX.Position,GridSystem3D.CURRENT_ITEM_HITBOX.Scale,GridHandler.CURRENT_ROTATION,false);
		}
    }
}
	...
	*/
	...
	public static Vector3 ClampObjectOnVoxel(Node3D _Voxel, Vector3 _ObjectPosition, Vector3 _NormalizedObjectSize, List<string> _Directions) {
		...
		CheckBoundsSide(new Vector3(
			FinalizedVector.X + (_NormalizedObjectSize.X / 2) - (GridHandler.CELL_SIZE / 2),
			_ObjectPosition.Y,
			FinalizedVector.Z + (_NormalizedObjectSize.Z / 2) - (GridHandler.CELL_SIZE / 2)
		),"Max_X");
		CheckBoundsSide(new Vector3(
			FinalizedVector.X + (_NormalizedObjectSize.X / 2) - (GridHandler.CELL_SIZE / 2),
			_ObjectPosition.Y,
		...
		if(CurrentVoxel != null && (bool)CurrentVoxel.GetMeta("VoxelIsPurchased",false)) {
			...
			if(ClosestVoxel != null) {
				...
				void DiffirentailVoxelMagnitude() {
					...
					Vector3 DiffirentialPosition = (LastSnappedToVoxel.Position - ClosestVoxel.Position) / (GridHandler.CELL_SIZE * GridHandler.VOXEL_CLUSTER_SIZE);
	...
	public bool CollideWithOtherEntities(Vector3 _ObjectPositon, Vector3 _ObjectSize, float _CurrentYRotation) {
		...
		float HitboxOffset = GridHandler.CELL_SIZE / 4; // system was being annoying
		...
		foreach(Node3D PlacedObject in GridHandler.NODE_PLACED_STORAGE.GetChildren()) {
	...
	public void Update(double _DeltaTime) {
		...
		Vector3 RayCastCursorPosition = GridHandler.RayCastFromCursor(new Godot.Collections.Array<Godot.Rid>{},"FloorColision");

		Vector3 RasterizedRayCastPosition = RasterizeObjectPosition(RayCastCursorPosition, CURRENT_ITEM_HITBOX.Scale, GridHandler.CURRENT_ROTATION, GridHandler.CELL_SIZE);
		Vector3 NormalizedRotation = NormalizeScale(CURRENT_ITEM_HITBOX.Scale,GridHandler.CURRENT_ROTATION);

		// Do this shit so that 1 by 3 dont unalign themselve from the grid
		Vector2 UnevenOffsetExtraction = new Vector2(
			Math.Abs(NormalizedRotation.X) % (GridHandler.CELL_SIZE * 2) == GridHandler.CELL_SIZE ? GridHandler.CELL_SIZE / 2 : 0,
			Math.Abs(NormalizedRotation.Z) % (GridHandler.CELL_SIZE * 2) == GridHandler.CELL_SIZE ? GridHandler.CELL_SIZE / 2 : 0
		);
		...
		NormalizedPosition = new Vector3(NormalizedPosition.X,GridHandler.CELL_HEIGHT + (CurrentSelectedItem.Scale.Y / 2),NormalizedPosition.Z);

		// Collisions
		NormalizedPosition = CollideWithBoundaries(RayCastCursorPosition,NormalizedPosition,CURRENT_ITEM_HITBOX.Scale,GridHandler.CURRENT_ROTATION);

		// We Set The Garbage to the actual hitbox and not the model itself
		// Done for springs
		CURRENT_ITEM_HITBOX.RotationDegrees = new Vector3(0,GridHandler.CURRENT_ROTATION,0);
		CURRENT_ITEM_HITBOX.Position = NormalizedPosition;

		// Visualization and collision stuff
		GridHandler.IS_COLLIDING = CollideWithOtherEntities(NormalizedPosition,CURRENT_ITEM_HITBOX.Scale,GridHandler.CURRENT_ROTATION);
		GridVisualization.Update(NormalizedPosition,CURRENT_ITEM_HITBOX.Scale);
		GridVisualization.CheckCollisions();

		if(GridHandler.SMOOTH_PLACING) {
			...
			RotationSpring.SetGoal(new Vector3(0,GridHandler.CURRENT_ROTATION,0));
			...
			CURRENT_ITEM_PREFAB.RotationDegrees = new Vector3(0,GridHandler.CURRENT_ROTATION,0);;
		}
	}
	...
	void ConvertVoxelsToGrid(bool _CurrentState) {
		foreach((string VoxelCords,Node3D Voxel) in VoxelGridHandler.VOXEL_GRID_LIBRARY) {
			if((bool)Voxel.GetMeta("VoxelIsPurchased")) {
				MeshInstance3D PlaneGridVoxel = VoxelGridHandler.VOXEL_PLANE_LIBRARY[VoxelCords];
		...
		if(_CurrentState) {
			foreach((string _,MeshInstance3D Tile) in GridVisualization.GRID_TILE_STORAGE) {
	...
	public void Enable() {
		...
		if(CurrentSelectedItemScene != null) {
			...
			GridHandler.NODE_PLACING_STORAGE.AddChild(CurrentSelectedItem);

			(CURRENT_ITEM_HITBOX,CURRENT_ITEM_PREFAB) = FindNesecaryNodes(CurrentSelectedItem);
			GridVisualization = new GridVisualization(CurrentSelectedItem.Name, CURRENT_ITEM_HITBOX.Scale);
		}
		GridHandler.IS_PLACING = true;
	...
	public void Disable() {
		...
		GridVisualization.Remove();
		CanUsePlacingUpdateFunction = false;
		GridHandler.IS_PLACING = false;
	...
	public Node3D Place(Vector3 _Position, Vector3 _Scale, float _Rotation, bool _Override) {
		...
		if(CurrentSelectedItem != null || _Override) {
			...
			if(!GridHandler.IS_COLLIDING && !ObjectIsColliding) {
				Node3D DesiredPlacingItem = CurrentSelectedItemScene.Instantiate<Node3D>();				
				GridHandler.NODE_PLACED_STORAGE.AddChild(DesiredPlacingItem);
				Node ItemMeta = GetItemMetaData(DesiredPlacingItem);

				DesiredPlacingItem.Name = GridHandler.ITEM_NAME + "_";
			
				DesiredPlacingItem.Position = new Vector3(_Position.X,GridHandler.CELL_HEIGHT,_Position.Z);
	...
	public void Rotate(int? _RotationAmount) {
		if(_RotationAmount != null) {
			GridHandler.CURRENT_ROTATION = (int)_RotationAmount;
		} else {
			GridHandler.CURRENT_ROTATION += GridHandler.ROTATION_AMOUNT;
		}
	}
	...
	public GridSystem3D(string _ItemName) {
		if(!File.Exists(ProjectSettings.GlobalizePath("res://") + "packages/Placeable/" + _ItemName + ".tscn")) {
			GridHandler.ITEM_NAME = "";
			return;
		} else {
			GridHandler.ITEM_NAME = _ItemName;
			CurrentSelectedItemScene = GD.Load<PackedScene>(GridHandler.PATH_TO_ITEM_RESOURCES + _ItemName + ".tscn");
		}
	}
}