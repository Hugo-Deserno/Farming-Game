...
public partial class VoxelGridHandler : Node {
    // OBjects
	...
	private void SetupScene() {
		...
		RayCastFloorScanner.Position = new Vector3(VoxelGridCenter.X,GridHandler.CELL_HEIGHT,VoxelGridCenter.Z);

		// Position The Boundaries
		CameraBoundaries.GetChild<Node3D>(0).Position = new Vector3(Max_X,GridHandler.CELL_HEIGHT,VoxelGridCenter.Z);
        CameraBoundaries.GetChild<Node3D>(1).Position = new Vector3(Min_X,GridHandler.CELL_HEIGHT,VoxelGridCenter.Z);
        CameraBoundaries.GetChild<Node3D>(2).Position = new Vector3(VoxelGridCenter.X,GridHandler.CELL_HEIGHT,Max_Y);
        CameraBoundaries.GetChild<Node3D>(3).Position = new Vector3(VoxelGridCenter.X,GridHandler.CELL_HEIGHT,Min_Y);
	}
...
public class VoxelGrid {
    ...
    static (Dictionary<string,Vector3>,Dictionary<string,Vector3>) GetVoxelInformation(Node3D _Voxel) {
        float VoxelIncrement = (float)Math.Round(GridHandler.VOXEL_CLUSTER_SIZE * GridHandler.CELL_SIZE,2);
    ...
    static void DrawVisualizationLine(Vector3 _Startpoint, Vector3 _Direction, string _VoxelName, Color _LineColor) { // direction should be to normal direction
        ...
        VoxelGridHandler.NODE_VOXEL_VISUALIZATION_STORAGE.AddChild(VoxelLineDraw);
    ...
    static void IndicatedActiveLandPiece(Vector3 _VoxelPosition, string _InstanceName) {
        ...
        VoxelActivationState.Scale = new Vector3(GridHandler.VOXEL_CLUSTER_SIZE * GridHandler.CELL_SIZE / 2,1,GridHandler.VOXEL_CLUSTER_SIZE * GridHandler.CELL_SIZE / 2);
        ...
        VoxelGridHandler.NODE_VOXEL_VISUALIZATION_STORAGE.AddChild(VoxelActivationState);
    ...
    static void CreateVoxelGridInstance(Vector3 _VoxelPosition) {
        ...
        // material Junk
        FillMaterial.Transparency = Godot.BaseMaterial3D.TransparencyEnum.Alpha;
        FillMaterial.AlbedoTexture = GD.Load<Texture2D>(GridHandler.PATH_TO_CELL_TEXTURE);
        ...
        GridHandler.NODE_VISUAL_STORAGE.AddChild(VoxelPlane);
        VoxelPlane.Name = _VoxelPosition.ToString();

        VoxelGridHandler.VOXEL_PLANE_LIBRARY.Add(VoxelPlane.Position.ToString(),VoxelPlane);
    }

     // Visualizes The Voxel Grid
    public static void VisualizeVoxelGrid(bool _CurrentState) {
        foreach(MeshInstance3D VisualVoxel in VoxelGridHandler.NODE_VOXEL_VISUALIZATION_STORAGE.GetChildren()) {
        ...
        if(_CurrentState) {
            foreach((string _,Node3D Voxel) in VoxelGridHandler.VOXEL_GRID_LIBRARY) {
                ...
                foreach((string Direction, Vector3 DirectionalVector) in DirectionalDictonary) {
                    if(VoxelGridHandler.VOXEL_GRID_LIBRARY.ContainsKey(DirectionalVector.ToString())) {
    ...
    public static void VisualizeNormalGrid(bool _CurrentState) {
        ...
        if(_CurrentState) {
            ...
            float VoxelIncrement = (float)Math.Round(GridHandler.VOXEL_CLUSTER_SIZE * (float)GridHandler.VOXEL_CLUSTER_AMOUNT,2);
            float VoxelIncrementOffset = (float)Math.Round(GridHandler.VOXEL_CLUSTER_SIZE * GridHandler.CELL_SIZE,2);
            ...
            for(float Collum = 0; Collum <  VoxelIncrement + 1; Collum++) {
                DrawVisualizationLine(
                    new Vector3(
                        ...
                        GridHandler.CELL_HEIGHT - 0.01f,
                        VoxelIncrementOffset / 2
                    ),
                    new Vector3(
                        0,
                        0,
                        (VoxelIncrement * GridHandler.CELL_SIZE) / 1.25f // 1.25 cuz draw line does /1.25
                    ),"CellGrid",Color.Color8(255,255,255)
                );
                CurrentXAxis += GridHandler.CELL_SIZE;
            }
            ...
            for(float Rows = 0; Rows <  VoxelIncrement + 1; Rows++) {
                ...
                CurrentYAxis += GridHandler.CELL_SIZE;
            }
            /*
            // X
    ...
    */
    ...
    public static void DefineVoxelNeighbours() {
        foreach((string _,Node3D Voxel) in VoxelGridHandler.VOXEL_GRID_LIBRARY) {
            ...
            foreach((string Direction, Vector3 DirectionalVector) in DirectionalDictonary) {                
                if(VoxelGridHandler.VOXEL_GRID_LIBRARY.ContainsKey(DirectionalVector.ToString())) {
                    Node3D VoxelNeighbour = VoxelGridHandler.VOXEL_GRID_LIBRARY[DirectionalVector.ToString()];
    ...
    public static Node3D GetVoxelFromPosition(Vector3 _InstancePosition) {
        ...
        VoxelLizedVector = new Vector3(VoxelLizedVector.X,GridHandler.CELL_HEIGHT,VoxelLizedVector.Z);

        if(VoxelGridHandler.VOXEL_GRID_LIBRARY.ContainsKey(VoxelLizedVector.ToString())) {
            Node3D VoxelizedVectorNode = VoxelGridHandler.VOXEL_GRID_LIBRARY[VoxelLizedVector.ToString()];
    ...
    */
    ...
    public static void MergeGridToNode3D() {
        ...
        foreach(var Object in VoxelGridHandler.PlacingSystemStorage.GetChildren()) {
            VoxelGridHandler.PlacingSystemStorage.RemoveChild(Object);
            PlacingStorageNode.AddChild(Object);
        }
        VoxelGridHandler.PlacingSystemStorage.QueueFree();

        VoxelGridHandler.ResourcesNode.AddChild(PlacingStorageNode);
    ...
    public static void CreateNewVoxelGrid() {
        ...
        string AssignVoxelID() {
            ...
            string GenerateRandomVoxelID() {
                float VoxelAmountCubed = (float)Math.Pow(GridHandler.VOXEL_CLUSTER_AMOUNT,2);
            ...
            if(VoxelGridHandler.VOXEL_GRID_LIBRARY.ContainsKey(VoxelSugestedId) is false) {
        ...
        foreach((int _, Variant Data) in VoxelData) {
            ...
            VoxelGridHandler.NODE_VOXEL_STORAGE.AddChild(Voxel);
            ...
            VoxelGridHandler.VOXEL_GRID_LIBRARY.Add(Voxel.Position.ToString(),Voxel);
            CreateVoxelGridInstance(Voxel.Position);
        }

        // Does the actual voxiliation
        for(float Collum = 0; Collum < GridHandler.VOXEL_CLUSTER_AMOUNT; Collum++) {
            for(float Rows = 0; Rows < GridHandler.VOXEL_CLUSTER_AMOUNT; Rows++) {
                Vector3 VoxelPosition = new Vector3(
                    (float)Math.Round((Collum + 1) * (GridHandler.VOXEL_CLUSTER_SIZE * GridHandler.CELL_SIZE),2),
                    GridHandler.CELL_HEIGHT,
                    (float)Math.Round((Rows + 1) * (GridHandler.VOXEL_CLUSTER_SIZE * GridHandler.CELL_SIZE),2)
                );

                if(!VoxelGridHandler.VOXEL_GRID_LIBRARY.ContainsKey(VoxelPosition.ToString())) {
                    Node3D Voxel = new Node3D();
        ...
        VoxelGridHandler.BoundaryNodes.GetChild<Node3D>(0).Position = new Vector3(Max_X,GridSystem3D.GridOffsetWorldSpace.Y,0);
        VoxelGridHandler.BoundaryNodes.GetChild<Node3D>(1).Position = new Vector3(Min_X,GridSystem3D.GridOffsetWorldSpace.Y,0);
        VoxelGridHandler.BoundaryNodes.GetChild<Node3D>(2).Position = new Vector3(0,GridSystem3D.GridOffsetWorldSpace.Y,Max_Y);
        VoxelGridHandler.BoundaryNodes.GetChild<Node3D>(3).Position = new Vector3(0,GridSystem3D.GridOffsetWorldSpace.Y,Min_Y);

        // Setup The RayCastingFloor
        VoxelGridHandler.VoxelFloorRayCastScanner.Scale = new Vector3(Max_X - Min_X,1,Max_Y - Min_Y);
        VoxelGridHandler.VoxelFloorRayCastScanner.Position = GridSystem3D.GridOffsetWorldSpace;
        */
