...
public partial class CommandConsole : Node
{
	...
	*/
	...
	// privates
	private static MouseRect MouseInUi;
    ...
    public override void _Ready() {
		...
		//privs
		MouseInUi = new MouseRect(CONSOLE,GetViewport());
    ...
    public override void _Input(InputEvent _InputEvent) {
		if(_InputEvent is InputEventMouseMotion MouseEvent) {
			if(CONSOLE.Visible) {
				MouseInUi.Step(MouseEvent.Position);
			}
		} else if(_InputEvent is InputEventMouseButton MouseButton) {
			if(MouseButton.ButtonIndex == Godot.MouseButton.Left) {
				if(MouseInUi.Get() == false && CONSOLE.HasFocus()) {
					CONSOLE.ReleaseFocus();
				}
			}
		} else if(_InputEvent is InputEventKey CurrentKey) {
	...
	public static void ExecuteStack(Dictionary<int,object> _Parameters, string _Command) {
		if(_Command == "LoadArchive") {
			...
			if(((Variant)((int)Convert.ToInt32(_Parameters[0]))).VariantType == Variant.Type.Int) {
				...
				// Load Scene
				PackedScene CurrentSelectedItemScene = GD.Load<PackedScene>("res://Packages/Objects/Game.tscn");
			...
			if(!GAME_ENABLED) {return;}
			PlacingGridHandler.PLACING_SYSTEM = new GridSystem3D((string)_Parameters[0]);
