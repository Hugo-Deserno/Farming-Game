    ...
    public override void _Process(double _DeltaTime) {
		...
		if(Input.IsActionJustPressed("Demolition_Start") && GetViewport().GuiGetFocusOwner() == null) {
			...
			if(DEMOLITION_TOOL_ENABLED) {
				...
				if(PlacingGridHandler.IS_PLACING && PlacingGridHandler.PLACING_SYSTEM != null) {
					PlacingGridHandler.PLACING_SYSTEM.Disable();
				}
...
public partial class DemolitionVisuals : DemolitionTool {
	...
	public static void HighlightDemolishedItem(Node3D _Object) {
		...
		foreach(Node3D Objects in PlacingGridHandler.NODE_PLACED_STORAGE.GetChildren()) {
	...
	public static void HighlightItem(Node3D _CurrentItem) {
		...
		if(_CurrentItem != null) {
			...
			int TileAmountX = (int)Math.Round(NormalizedScale.X / PlacingGridHandler.CELL_SIZE,0);
			int TileAmountZ = (int)Math.Round(NormalizedScale.Z / PlacingGridHandler.CELL_SIZE,0);
			...
			for(var XRows = 0; XRows < TileAmountX; XRows++) {
				for(var ZRows = 0; ZRows < TileAmountZ; ZRows++) {
					...
					Vector3 RasterizedCellPosition = GridSystem3D.RasterizeObjectPosition(
						...
						PlacingGridHandler.CELL_SIZE
					);

					// Do this shit so that 1/2 size ratio dont unalign themselve from the grid
					Vector2 UnevenOffsetExtraction = new Vector2(
						Math.Abs(NormalizedScale.X) % (PlacingGridHandler.CELL_SIZE * 2) == PlacingGridHandler.CELL_SIZE ? PlacingGridHandler.CELL_SIZE / 2 : 0,
						Math.Abs(NormalizedScale.Z) % (PlacingGridHandler.CELL_SIZE * 2) == PlacingGridHandler.CELL_SIZE ? PlacingGridHandler.CELL_SIZE / 2 : 0
					);
					...
					MeshInstance3D Tile = CreateGridTile(new Vector2(RasterizedCellPosition.X,RasterizedCellPosition.Z) + new Vector2(XRows * PlacingGridHandler.CELL_SIZE + (PlacingGridHandler.CELL_SIZE / 2),ZRows * PlacingGridHandler.CELL_SIZE + (PlacingGridHandler.CELL_SIZE / 2)));
